...
class ViewController: UITableViewController {
    ...
    func filterOn(_ userString: String) {
        performSelector(inBackground: #selector(filter), with: userString)
    }
    
    @objc func filter(userString: String) {
        ...
        DispatchQueue.main.async {
            [weak self] in
            self?.clearBarButton = UIBarButtonItem(title: "Clear", style: .done, target: self, action: #selector(self?.clearFilter))
            if let clearBarButton =  self?.clearBarButton {
                 self?.navigationItem.rightBarButtonItems = [clearBarButton]
            }
            self?.tableView.reloadData()
        }
    }
