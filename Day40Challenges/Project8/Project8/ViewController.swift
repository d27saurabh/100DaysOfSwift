...
//  Project8
//
//  Created by Saurabh Deshpande on 8/25/20.
        ...
        if let levelFileURL = Bundle.main.url(forResource: "level\(level)", withExtension: "txt") {
            ...
            if let levelContents = try? String(contentsOf: levelFileURL) {
                ...
                for (index, line) in lines.enumerated() {
                    ...
                    //assign first part as answer
                    let answer = parts[0]
                    //assign second for as clue
                    let clue = parts[1]
                    
                    //add the clue to the clue string
                    clueString += "\(index + 1). \(clue)\n"
                    
                    //replace the | with empty to get the solution
                    let solutionWord = answer.replacingOccurrences(of: "|", with: "")
                    //assign that solution to the solution string
                    solutionString += "\(solutionWord.count) letters\n"
                    //add the solution to the solutions array
                    solutions.append(solutionWord)
                    
                    //get the answer bits seperated by | in an array
