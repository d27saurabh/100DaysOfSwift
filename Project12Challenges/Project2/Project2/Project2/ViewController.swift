...
//        present(ac, animated: true)
        ...
        if questionsAsked % 10 == 0 {
            var am = UIAlertController()
            
            if highScore > score {
                defaults.setValue(highScore, forKey: "highScore")
                am = UIAlertController(title: "Final Score", message: "Score:\(score) \n HighScore:\(highScore!)", preferredStyle: .alert)
            } else {
                highScore = score
                am = UIAlertController(title: "Final Score", message: "New highscore: \(highScore!)", preferredStyle: .alert)
            }
    ...
    @objc func showScore() {
        let am = UIAlertController(title: "Final Score", message: "Score:\(score) \n HighScore:\(highScore!)", preferredStyle: .alert)
